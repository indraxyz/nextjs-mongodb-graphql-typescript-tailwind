 ...
 * Mendukung berbagai format input dan timezone Indonesia
 */
...
export const formatDateTime = (
  ...
  try {
    // Handle berbagai format date string
    let date: Date;

    if (typeof dateString === "string") {
      // Jika sudah dalam format ISO string, langsung parse
      if (dateString.includes("T") && dateString.includes("Z")) {
        date = new Date(dateString);
      } else if (dateString.includes("T")) {
        // Handle format dengan timezone offset
        date = new Date(dateString);
      } else {
    ...
    // Format untuk Indonesia dengan timezone
